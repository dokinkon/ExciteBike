...
public abstract class FSMState
{
    ...
    public void AddTransition(string transitionName, string stateName)
    {
        ...
        if (map.ContainsKey(transitionName))
        {
            Debug.LogError("FSMState ERROR: State " + _stateName + " already has transition " + transitionName + 
                           "Impossible to assign to another state");
            return;
        }
 
        map.Add(transitionName, stateName);
    }
    ...
    public void DeleteTransition(string transitionName)
    {
        ...
        if (map.ContainsKey(transitionName))
        {
            map.Remove(transitionName);
            return;
        }
        Debug.LogError("FSMState ERROR: Transition " + transitionName + " passed to " + _stateName + 
    ...
    /// </summary>
    public string GetOutputState(string transitionName)
    {
        // Check if the map has this transition
        if (map.ContainsKey(transitionName))
        {
            return map[transitionName];
        }
        return null;
    }
